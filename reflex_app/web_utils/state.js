...
// ---- PreConsult patch: dead-session event replay ---------------------------
// Keeps the most recently emitted stateful event so that if the Engine.IO
// session dies server-side while the client still believes its socket is
// connected (the POST returns a 400 "Invalid session"), we can re-handshake
// and replay the event exactly once. Matches the server-side
// _StaleSocketSessionMiddleware recovery in preconsult.py.
let _lastEmit = null; // { event, name, at }
const _REPLAY_WINDOW_MS = 15000;
// ---------------------------------------------------------------------------
...
export const applyEvent = async (event, socket, navigate, params) => {
  ...
  if (socket) {
    // PreConsult patch: remember the in-flight stateful event so it can be
    // replayed if this emit's session is dead server-side. Bounded buffer.
    const isStatefulEvt = event.name && event.name.startsWith("reflex___state");
    if (isStatefulEvt) {
      _lastEmit = { event, name: event.name, at: Date.now() };
    }
...
export const connect = async (
  ...
  // PreConsult patch: replay a state event lost to a dead Engine.IO session.
  // The reflex client believes its socket is connected, but the server has
  // already closed the session (a POST returns 400 "Invalid session"). Re-queue
  // the newest in-flight state event once and force a fresh handshake; the
  // connect handler drains event_queue and replays it. Used by the `error`
  // handler registered below.
  socket.current.replayLostEvent = () => {
    if (!_lastEmit || !_lastEmit.event) {
      return false;
    }
    if (Date.now() - _lastEmit.at > _REPLAY_WINDOW_MS) {
      _lastEmit = null;
      return false;
    }
    const event = _lastEmit.event;
    _lastEmit = null; // only ever replay once per in-flight event.
    // Dedupe: don't re-queue if it's already sitting in the queue.
    if (event_queue.some((e) => e && e.name === event.name)) {
      return true;
    }
    // Re-queue at the front and force a fresh handshake so the replay goes out on
    // a healthy sid. The connect handler drains event_queue on reconnect. We must
    // tear down the dead session even if the client still believes it is connected
    // (the socket.io client only notices the transport error lazily).
    event_queue.unshift(event);
    if (socket.current) {
      try {
        socket.current.io.opts.query = { token: getToken() };
      } catch (_e) {
        // best-effort token refresh
      }
      socket.current.disconnect();
      // reflex's reconnect() sets wait_connect + rehydrate + connect(); it guards
      // on !connected && !wait_connect, so call it after disconnect().
      socket.current.reconnect();
    }
    return true;
  };
  ...
  // PreConsult patch: recover a state event lost to a dead Engine.IO session.
  // socket.io-client surfaces a transport error on an already-connected socket
  // as a plain `error` (not `disconnect`/`connect_error`). This is the exact
  // failure mode for a POST that returns 400 "Invalid session": replay the
  // pending event once after re-handshaking.
  ...
  socket.current.on("event", async (update) => {
    // PreConsult patch: a healthy round-trip proves the session (sid) is alive;
    // drop the pending replay so a normal interaction never double-fires.
    _lastEmit = null;
