...
def test_rxconfig_websocket_transport_is_default(monkeypatch):
    # We deploy with `--no-use-http2`, so the frontend speaks HTTP/1.1, over which
    # the browser CAN upgrade to a WebSocket (the old "cannot upgrade from HTTP/2"
    # caveat no longer applies). Long-polling over Cloud Run proved unstable, so
    # the event transport defaults to websocket for "Start Preparing" and all
    # state mutations on the deployed host.
    import reflex_app.rxconfig as rxconfig
...
def test_rxconfig_transport_is_env_overridable(monkeypatch):
    # Allow rolling back to polling (or a staging origin) without a code
